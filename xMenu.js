...
var xMenu = function (param) {
    ...
    $.each(arg.itens, function (name, props) {
        ...
        var divIcon = '<div></div>';
        divIcon = '<div><i class="' + props.icon + '"></i></div>';
//        divIcon = '<div><i class="fa ' + props.icon + '"></i></div>';
    ...
    $('body').append(ul).append('<div style="display: none;" id="pnBackgroudxMenu"></div>');
    ...
    if (!arg.buttonRight) {
        $(document).on("contextmenu", arg.id, function (event) {
            ...
            $('#pnBackgroudxMenu').css('display', 'block');

            window.location.hash = idxMenu;
            setTimeout(function () {
                hashxMenu.id = idxMenu;
                hashxMenu.status = true;
            }, 200);
...
//        para fechar
    $(document).on('click', '#pnBackgroudxMenu', function (e) {
        $(this).css('display', 'none');
        $('#popup_' + idxMenu).hide('fast');
    });

    $(document).on('click', '.xMenu', function (e) {
        $('#pnBackgroudxMenu').css('display', 'none');
        $('#popup_' + idxMenu).hide('fast');
    });
